...
import CardHand from "../gameObjects/CardHand";
import AnimationDispatcher from "../helpers/AnimationDispatcher";
...
export default class GameTableTestScene extends Phaser.Scene
{
    ...
    create() {
        ...
        // anuntam ca e gata scena
        window.$socket.emit("room/sceneReady");

        window.$socket.on("room/roomData", (data:IRoom.Data) => {
            console.log(data);
            console.log(window.getSessionToken());
            let you: IPlayer.Data = data.players.find((p:IPlayer.Data) => p.id == window.getSessionToken());
            let enemy: IPlayer.Data = data.players.find((p:IPlayer.Data) => p.id != window.getSessionToken());
            you.cards.forEach((card:ICard.Data) => card.isMyCard = true);
            myPack._.addCards(you.cards.filter((card:ICard.Data) => card.location == "inPack"));
            myHand._.addCards(you.cards.filter((card:ICard.Data) => card.location == "inHand"));
        });

        window.socket.on("room/waitingForOpponentToJoin", () => {
            alert("Waiting for opponent to join");
        });

        window.socket.on("room/eventsQueue", (data:any) => {
            AnimationDispatcher.dispatch(data);
            //AnimationDispatcher.dispatchInLine(data);
        });
/*
        myPack._.addCards([
